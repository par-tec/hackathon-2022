...
import {Battlesnake, Coord} from "../../starter-kits/starter-snake-typescript/types";
...
function getMovesWeights(head: Coord, snakes: Battlesnake[], safeMoves: string[]) {
  // Weights go from -1 to 15, where -1 is highly discouraged.
  let moveWeights: { [key:string]: number; } = {
    up: 0,
    down: 0,
    left: 0,
    right: 0
...
function move(gameState: GameState): MoveResponse {
  ...
  let nextMove = <string>Object.keys(weights).sort((prev, next) => weights[prev] - weights[next]).pop();
